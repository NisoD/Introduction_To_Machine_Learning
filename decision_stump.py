...
"""selects a feature of the design matrix X and a corresponding
threshold value that achieve the best classification result when thresholding only with regard
to that feature"""
class DecisionStump(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        for i in range(features):
            values = X[:, i]
            for sign in [-1, 1]:
                threshold, t_error = self._find_threshold(values, y, sign)
                if t_error < error:
                    error = t_error
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        # if feature value is higher than threshold 1 else -1
